...
// dtor
//---------------------------------------------------------------------
PyStand::~PyStand() { FreeLibrary(_hDLL); }
...
std::wstring PyStand::GetPythonInstallPath() {
    std::wstring installPath;
    HKEY hKey;
    // 尝试打开 HKEY_LOCAL_MACHINE 下 "Software\TaijiControl" 注册表项
    LONG lResult = RegOpenKeyEx(HKEY_LOCAL_MACHINE, "Software\\TaijiControl\\PythonEnv", 0,
    ...
    // 读取 "InstallPath" 值
    lResult =
        RegQueryValueEx(hKey, "InstallPath", NULL, &dwType, (LPBYTE)szInstallPath, &dwBufferSize);
    if (lResult != ERROR_SUCCESS) {
...
bool PyStand::CheckEnviron(const wchar_t *rtp) {
    ...
    //_runtime = path;
    _runtime = GetPythonInstallPath();
    if (_runtime.empty()) {
        _runtime = GetInstallPath();    
    }
...
const char *init_script =
    ...
    "sys.path.append(PYSTAND_RUNTIME + \"\\Lib\\site-packages\")\n"
